...
- Определи путь базы знаний `<vault_root>` (локальная папка `.vault/` в корне проекта или глобальная папка `<obsidian_projects>/<name>/` из конфига).
...
1. **Индекс-слой (дёшево, основной источник для «Что сделано за период»):**
   ...
   - Каждая строка индекса — готовый однострочник сессии (формат `- <дата> · **<project>** · [[Sessions/<id>|<тема/что сделано>]]` или аналогичный). **ID сессии = `<id>`** (имя файла), он же — ссылка на тело.
